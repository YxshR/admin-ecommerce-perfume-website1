...
// The client component that handles all the functionality
export default function OrderDetailClient({ params }: { params: { id: string } }) {
  ...
  const formatOrderData = (orderData: any): Order => {
    ...
    let subtotalPrice = 0;
    if (items.length > 0) {
      subtotalPrice = items.reduce((sum: number, item: any) => {
        return sum + ((item.price || 0) * (item.quantity || 1));
      }, 0);
    } else {
      subtotalPrice = orderData.itemsPrice || 0;
    }
    
    // Get shipping price from API response or calculate based on subtotal
    const shippingPrice = orderData.shippingPrice ?? (subtotalPrice >= 500 ? 0 : 70);
    
    // Calculate total price with shipping
    const totalPrice = orderData.totalPrice || orderData.total || (subtotalPrice + shippingPrice);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-white shadow rounded-lg p-4 sm:p-6">
          ...
          <div className="space-y-3">
            ...
            <p className="text-gray-700"><strong>Subtotal:</strong> ₹{order.itemsPrice?.toFixed(2) || 'N/A'}</p>
            <p className="text-gray-700">
              <strong>Shipping:</strong> {(order.itemsPrice || 0) >= 500 ? 
                <span className="text-green-600">Free (Order above ₹500)</span> : 
                `₹${order.shippingPrice?.toFixed(2) || '70.00'}`
              }
            </p>
            <p className="text-gray-700 font-medium"><strong>Total Amount:</strong> ₹{order.totalPrice?.toFixed(2) || 'N/A'}</p>
          </div>
        </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tfoot className="bg-gray-50">
              ...
              <tr>
                ...
                <td colSpan={2} className="px-3 sm:px-6 py-3 whitespace-nowrap text-sm font-medium md:hidden">
                  {(order.itemsPrice || 0) >= 500 ? (
                    <span className="text-green-600">Free</span>
                  ) : (
                    <span>₹{order.shippingPrice?.toFixed(2) || '70.00'}</span>
                  )}
                </td>
                <td className="px-3 sm:px-6 py-3 whitespace-nowrap text-sm font-medium hidden md:table-cell">
                  {(order.itemsPrice || 0) >= 500 ? (
                    <span className="text-green-600">Free</span>
                  ) : (
                    <span>₹{order.shippingPrice?.toFixed(2) || '70.00'}</span>
              ...
              <tr className="bg-gray-100">
                <th scope="row" colSpan={4} className="px-3 sm:px-6 py-3 text-right text-sm font-semibold text-gray-900 hidden md:table-cell">
                  Total Amount
                </th>
                <th scope="row" colSpan={1} className="px-3 sm:px-6 py-3 text-right text-sm font-semibold text-gray-900 md:hidden">
                  Total
                </th>
                <td colSpan={2} className="px-3 sm:px-6 py-3 whitespace-nowrap text-sm text-gray-900 font-semibold md:hidden">
                  ₹{order.totalPrice?.toFixed(2) || 'N/A'}
                </td>
                <td className="px-3 sm:px-6 py-3 whitespace-nowrap text-sm text-gray-900 font-semibold hidden md:table-cell">
                  ₹{order.totalPrice?.toFixed(2) || 'N/A'}
                </td>
              </tr>
            </tfoot>
